...
def match_with_gaps(my_word, other_word):
    ...
    '''
    # FILL IN YOUR CODE HERE AND DELETE "pass"
    my_word = my_word.replace(" ", "") #removed the spaces to ensure correct indexing
    if len(my_word) != len(other_word):
        return False
    
    for i in range(len(my_word)):
        if my_word[i] != '_' and my_word[i] != other_word[i]:  # Use index i for comparison
            return False
        if my_word[i] == '_' and other_word[i] in my_word:  # to avoid repeated letters in the hidden word
            return False
...
def show_possible_matches(my_word):
    ...
    '''
    # FILL IN YOUR CODE HERE AND DELETE "pass"
    matched_words = []  # Store matching words
    for word in wordlist:
        if match_with_gaps(my_word, word):
            matched_words.append(word)

    if matched_words:  # Check if any matches were found
        print("Possible matches:", ", ".join(matched_words))  # Prints all matching words in one string separated by ", "
    else:
        print("No matches found")  # Print if no matches were found 
